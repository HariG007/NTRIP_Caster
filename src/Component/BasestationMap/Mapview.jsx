...
const MapView = () => {
  ...
  const generateRandomChange = () => {
    const change = 0.0001;
    const newLatitude = userLocation.latitude + (Math.random() * change - change / 2);
    const newLongitude = userLocation.longitude + (Math.random() * change - change / 2);
  ...
  const addUserMarkers = (locations) => {
    if (map && userLocation.latitude && userLocation.longitude) {
      ...
      if (!map.getUserLocationMarker) {
        ...
        map.addControl(userMarker); // Try using 'addControl' instead of 'addTo'
  
        map.getUserLocationMarker = userMarker;
      } else {
        map.getUserLocationMarker.setLngLat(newYellowMarkerCoordinatesArray);
      }
    }
  };
  useEffect(() => {
